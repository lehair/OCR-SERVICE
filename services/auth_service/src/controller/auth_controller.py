...
# Dùng pbkdf2_sha256 để hash mật khẩu (tránh lỗi bcrypt)
...
class DocsStatsResponse(BaseModel):
  total_docs: int                  # tổng số bản ghi document
  by_type: Dict[str, int]          # thống kê theo loại tài liệu
  by_language: Dict[str, int]      # thống kê theo ngôn ngữ
...
# ====== API THỐNG KÊ ĐĂNG NHẬP (cho Dashboard Login Stats) ======
...
# ====== API THỐNG KÊ DOCUMENT (cho Dashboard tài liệu) ======
@router.get("/stats/docs", response_model=DocsStatsResponse)
def document_stats(db: Session = Depends(get_db)):
  # 1. Tổng số document
  total_docs = db.query(Document).count()

  # 2. Đếm theo Loại tài liệu (doc_type)
  by_type_rows = (
    db.query(Document.doc_type, func.count().label("count"))
    .group_by(Document.doc_type)
    .all()
  )
  # Chuyển list [(doc_type, count), ...] -> dict {"CCCD": 40, "THE_SV": 30, ...}
  by_type = {doc_type: cnt for doc_type, cnt in by_type_rows}

  # 3. Đếm theo Ngôn ngữ (language)
  by_lang_rows = (
